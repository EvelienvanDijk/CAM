...
real(r8)           :: micro_mg_falspeed_factor       !
real(r8)           :: micro_mg_falspeed_temp         !
...
subroutine micro_mg_init( &
     ...
     micro_mg_falspeed_factor_in, micro_mg_falspeed_temp_in)
  ...
  real(r8), intent(in), optional :: micro_mg_falspeed_factor_in !
  real(r8), intent(in), optional :: micro_mg_falspeed_temp_in   !
  ...
  if (present(micro_mg_falspeed_factor_in)) then
   micro_mg_falspeed_factor = micro_mg_falspeed_factor_in
  else
  ...
  if (present(micro_mg_falspeed_temp_in)) then
   ...
   micro_mg_falspeed_factor = 1.0_r8
   micro_mg_falspeed_temp   = 0.0_r8
  endif
  if (masterproc) then
     write(iulog,*) 'Actual fallout speedup factor and threshold temperature: ', &
     micro_mg_falspeed_factor, micro_mg_falspeed_temp
  endif
  nccons = nccons_in
  nicons = nicons_in
  ncnst  = ncnst_in
